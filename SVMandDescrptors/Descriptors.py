...
def split_images_descriptors_train_test(base_described, test_proportion=0.2, cross_validation=False, *args):
    """creation of the training set and the test set
    INPUT : a list of arrays: (list of descriptors, targeted class, image) for all images of the base
    OUTPUT : list of lists of descriptors (X), list of targets (Y) [train]
             list of lists of descriptors (X), list of targets (Y), list of images (X_images)  [test]
    """

    if not cross_validation:

        descriptors_all = base_described[:, 0]
        y_all = base_described[:, 1]
        images_all = base_described[:, 2]
        indices = range(len(base_described))
        descriptors_train, descriptors_test, y_train, y_test, _, idx_test = train_test_split(descriptors_all, y_all,
                                                                                             indices, test_size=test_proportion,
                                                                                             shuffle=True)

        X_image_test = images_all[idx_test]

        return descriptors_train, y_train, descriptors_test, y_test, X_image_test
